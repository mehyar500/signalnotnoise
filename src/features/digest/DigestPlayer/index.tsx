      ...
      <GlassCard className="w-full max-w-2xl max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {paragraphs.map((paragraph, i) => (
            <p key={i} className={`leading-relaxed ${i === 0 ? 'text-white/50 text-sm' : 'text-white/80 text-sm'}`}>
              {paragraph}
            </p>
          ))}
        </div>
        ...
        <div className="p-5 border-t border-white/10">
          ...
          <p className="text-center text-white/30 text-xs mt-2">{digest.clusterCount} stories · {digest.articleCount} articles analyzed</p>
        </div>
      </GlassCard>
    </div>
  );
